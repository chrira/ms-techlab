...
```bash
$ oc create -f ./data/pipelines/simple-openshift-pipeline.yaml
```

Während OpenShift arbeitet, schauen wir uns noch die verwendete Konfigutationsdatei an: [data/pipelines/simple-openshift-pipeline.yaml](./data/pipelines/simple-openshift-pipeline.yaml)
...
```bash
oc policy add-role-to-group system:image-puller system:serviceaccounts:userXY-pipeline-dev -n userXY-buildpipeline
oc policy add-role-to-group system:image-puller system:serviceaccounts:userXY-pipeline-test -n userXY-buildpipeline
oc policy add-role-to-group system:image-puller system:serviceaccounts:userXY-pipeline-prod -n userXY-buildpipeline
```

Des Weiteren müssen wir nun dem `jenkins` Serviceaccount aus dem Projekt `userXY-buildpipeline` Rechte geben, damit er in den Projekten der Stages Ressourcen anlegen kann.

```bash
oc policy add-role-to-user edit system:serviceaccount:userXY-buildpipeline:jenkins -n userXY-pipeline-dev
oc policy add-role-to-user edit system:serviceaccount:userXY-buildpipeline:jenkins -n userXY-pipeline-test
oc policy add-role-to-user edit system:serviceaccount:userXY-buildpipeline:jenkins -n userXY-pipeline-prod
```
...
```bash
oc tag userXY-buildpipeline/application:latest userXY-buildpipeline/application:dev
oc tag userXY-buildpipeline/application:dev userXY-buildpipeline/application:test
...
```groovy
def project=""
def dev_project="userXY-pipeline-dev"
def test_project="userXY-pipeline-test"
def prod_project="userXY-pipeline-prod"
node {
    stage('Init') {
        project = env.PROJECT_NAME
    }
...
Ebenso haben wir weiter oben mit dem Befehl `oc new-app userXY-buildpipeline/application:stage -n userXY-pipeline-stage` im Hintergrund die nötigen Ressourcen angelegt. In unserem Fall `Service` und `DeploymentConfig`.
